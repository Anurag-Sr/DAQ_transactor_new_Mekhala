...
def sca_i2c_discover(sca):
    ...
    rocs = []
    sca.enableI2C(0x0003)  ############### Changed Sept 2023  ########################
    for bus_id in range(2):  #### set to 1, was 15 before, July2023, 8 i2c bus lines. Sept. 23, for B-boards: Put range to 2
        try:
...
def sca_i2c_create(rocs,sca):
    ...
    elif n == 2:   ############### Added Sept 2023: n=2  ########################
        print('[I2C] Identified B-type tileboard')
        roc_map = i2c_2ROC_map    
    elif n == 3: 
...
i2c_2ROC_map = {0x28: 'roc_s0', 0x38: 'roc_s1'}     ############### Added Sept 2023  ########################
...
gpio_tbtester_map = {'roc_s0':[('hgcroc_soft_rstB',0x4),       ############### Changed Sept 2023  ########################
     ('hgcroc_i2c_rstB',0x8), ('hgcroc_hard_rstB',0x10)],
     'roc_s1':[]}
...
def sca_gpio_discover(sca):
    lines = []
    sca.enableGPIO(True)
    gpio_map = gpio_tbtester_map  # New name for B-boards
    ret = {}
    ...
    for roc, line_names in gpio_map.items():
            ...
            for line in line_names:
                 lines.append(sca.gpio(line[0],line[1],False))
            ret[roc] = sca_gpio(lines)
    if ret: return ret
    else: raise('Missing Tileboard GPIO lines.')
    '''
    ###### this is the old setup with only one ROC ##############
