...
public:
  pedData(){;}
  pedData(int chip, int channel, int channeltype, int adc=0)
  {
    m_adc=adc;
    m_detid = DetectorId(FromPedData(), chip, channel, channeltype);
  }
  ~pedData(){;}
  inline int adc() const {return m_adc;}
  inline DetectorId detid() const {return m_detid;}
  friend std::ostream& operator<<(std::ostream& out,const pedData& rs);
private:
  DetectorId m_detid;
  int m_adc;
};
