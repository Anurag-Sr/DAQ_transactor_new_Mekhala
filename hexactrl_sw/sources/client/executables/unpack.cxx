...
int main(int argc, char** argv)
{
  ...
  try { 
    ...
    generic_options.add_options()
      ...
      ("outputType,t",   po::value<std::string>(&m_outputType)->default_value("root"), "data type option -> will decide what kind of unpacker is used and what kind of ntuple is saved; available choice : root, unpacked, delayscan, text")
  ...
  if( m_metaDataYaml.empty() ){
    ...
    else if( m_outputType=="text" ){
      writer = fac.Create("text",m_output);
      std::cout << "save text data in : " << *writer << std::endl;
    }
    else{
    ...
    std::cout << "save root data with yaml node in : " << *writer << std::endl;
  }
