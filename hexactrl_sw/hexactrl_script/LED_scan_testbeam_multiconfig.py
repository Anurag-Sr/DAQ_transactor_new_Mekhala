...
configs = ["roc_config_ConvGain12_chs.yaml"]
#configs = ["roc_config_ConvGain12_chs_ch0to18.yaml","roc_config_ConvGain12_chs_ch19to36.yaml","roc_config_ConvGain12_chs_ch37to54.yaml","roc_config_ConvGain12_chs_ch55to71.yaml"]
#configs = ["roc_config_ConvGain12_chs_ch19to36.yaml","roc_config_ConvGain12_chs_ch37to54.yaml","roc_config_ConvGain12_chs_ch55to71.yaml"]
#configs = ["roc_config_ConvGain12_chs_ch0to18.yaml"]
...
# added for ROCv3 configuration ------------------------
    ...
    for trim_val in range(trimstart, trimstop, trimstep):
        ...
        for key in i2csocket.yamlConfig.keys():
            if key.find('roc_s')==0:
                ...
                print(nestedConf)
                for chan in range(0,72):
                    
                    #print(nestedConf[key]['sc']['ch'][str(chan)])
                    #print(key,chan)
                    #sys.exit()
                    [nestedConf[key]['sc']['ch'][chan].update({'trim_inv': trim_val+i2csocket.yamlConfig[key]['sc']['ch'][chan]['trim_inv']}) for key in i2csocket.yamlConfig.keys() if key.find('roc_s')==0 ] 
    ...
    #=========================================================================================
        ...
        for BX in range(startBX, stopBX, stepBX):
            ...
            daqsocket.yamlConfig['daq']['menus']['calibAndL1A']['prescale']=15   
...
def sampling_scan(i2csocket,daqsocket, clisocket, basedir,device_name, injectionConfig, config,suffix=""):
    ...
    calibreq = 0x10
    bxoffset = 25 #  --------24 in mathias's script
    noofoffsets = 1
    ...
    config_name = config[config.find("ch"):config.find(".yaml")]
    ...
    odir = "%s/%s/LED_scan_%s/TB3_LED_scan_Rf14_ConvGain12_OV%iV_%imV_BXstart%i_BXstop%i_BXstep%i_Phsstart%i_Phsstop%i_Phsstep%i_trimstart%i_trimstop%i_trimstep%i_%s/"%( 
                                                                                    os.path.realpath(basedir), device_name, config_name, voltages, LEDvolt,
...
if __name__ == "__main__":
    ...
    for config in configFile:
        ...
        injectionConfig = {
            ...
            'injectedChannels' : [13,14,15,49,50,51],
            #'injectedChannels' : [10],
            'LEDvolt' : 6000,  # LED_BIAS (LED amplitude) in mV  #------------added for sampling scan ext
            'OV'    : 4   # SiPM overvoltage   #------------added for sampling scan ext
        }
        
        sampling_scan(i2csocket,daqsocket,clisocket,options.odir,options.dut,injectionConfig,config,suffix="")
