...
def toa_vref_scan_noinj(i2csocket,daqsocket,clisocket,basedir,device_name,scan_config,configFile):
    ...
    try:
        ...
        yaml_dict = scan_analyzer.findVref()
    ...
    cfg["roc_s0"]["sc"]["ReferenceVoltage"][0]["Toa_vref"] = yaml_dict["roc_s0"]["sc"]["ReferenceVoltage"][0]["Toa_vref"]
    cfg["roc_s0"]["sc"]["ReferenceVoltage"][1]["Toa_vref"] = yaml_dict["roc_s0"]["sc"]["ReferenceVoltage"][1]["Toa_vref"]
        
    configFile0 = configFile[:configFile.find(".yaml")]
    with open(configFile0+"_Vreftoa.yaml", "w") as o:
        yaml.dump(cfg, o)
    print("Saved new config file as:"+configFile0+"_Vreftoa.yaml")  
...
if __name__ == "__main__":
    ...
    scanConfig = {
        ...
        #'toa_vref' : [i for i in range(100,220,2)], #==============for A5 board
    }
    scanConfig['toa_vref'].sort(reverse=True)
    toa_vref_scan_noinj(i2csocket,daqsocket,clisocket,options.odir,options.dut,scanConfig,options.configFile)
