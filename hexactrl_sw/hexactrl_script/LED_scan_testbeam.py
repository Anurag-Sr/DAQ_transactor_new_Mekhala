...
# added for ROCv3 configuration ------------------------
    ...
    for trim_val in range(trimstart, trimstop, trimstep):
        ...
        for key in i2csocket.yamlConfig.keys():
            if key.find('roc_s')==0:
                ...
                print(nestedConf)
                nestedConf[key]['sc']['ch']['all']['trim_inv'] = trim_val#================================test21072023
                #for chan in range(0,72):#================================test21072023
                    
                    #print(nestedConf[key]['sc']['ch'][str(chan)])
                    #print(key,chan)
                    #sys.exit()
                    #[nestedConf[key]['sc']['ch'][chan].update({'trim_inv': trim_val+i2csocket.yamlConfig[key]['sc']['ch'][chan]['trim_inv']}) for key in i2csocket.yamlConfig.keys() if key.find('roc_s')==0 ] #================================test21072023
    ...
    #=========================================================================================
        ...
        for BX in range(startBX, stopBX, stepBX):
            ...
            daqsocket.yamlConfig['daq']['menus']['calibAndL1A']['prescale']=25   
...
def sampling_scan(i2csocket,daqsocket, clisocket, basedir,device_name, injectionConfig,suffix=""):
    ...
    trimstart=0
    trimstop=3  # was 3 excluded
    trimstep=1
    
    calibreq = 0x10
    bxoffset = 26 #  --------24 in mathias's script 
    noofoffsets = 2
    ...
    odir = "%s/%s/LED_scan/TB3_LED_scan_Rf12_Cf2_dacb0_ConvGain12_OV%iV_%imV_BXstart%i_BXstop%i_BXstep%i_Phsstart%i_Phsstop%i_Phsstep%i_trimstart%i_trimstop%i_trimstep%i_%s/"%( 
...
if __name__ == "__main__":
    ...
    injectionConfig = {
        ...
        'injectedChannels' : [1, 2, 3, 29, 31, 33, 35, 36, 38, 49, 51],
        #'injectedChannels' : [10],
        'LEDvolt' : 6600,  # LED_BIAS (LED amplitude) in mV  #------------added for sampling scan ext
