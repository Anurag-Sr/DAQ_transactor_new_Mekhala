...
config_dict = {	"sipm_roc0_onbackup0_gainconv4_trimtoaNEW_Board2.yaml":"ConvGain4_trimtoa",
            	...
            	"sipm_roc0_onbackup0_gainconv2_trimtoaNEW_Board1.yaml":"ConvGain2_trimtoa",
            	"sipm_roc0_onbackup0_gainconv4_trimtoaNEW_Board1.yaml":"ConvGain4_trimtoa",
            	"sipm_roc0_onbackup0_gainconv8_trimtoaNEW_Board1.yaml":"ConvGain8_trimtoa",
              	"sipm_roc0_onbackup0_gainconv12_trimtoaNEW_Board1.yaml":"ConvGain12_trimtoa",
              	"sipm_roc0_onbackup0_gainconv2_timingtests.yaml":"ConvGain2_timingtests_showers5_4GeV_CalSC1",
	            "sipm_roc0_onbackup0_gainconv4_trimtoaNEW_timingtests_Board2.yaml":"ConvGain4_timingtests_showers5_4GeV_CalSC1",
                "sipm_roc0_onbackup0_gainconv12_timingtests.yaml":"ConvGain12_timingtests_showers5_4GeV_CalSC1"
    }
...
def beam_run(i2csocket,daqsocket, clisocket, basedir,device_name, nruns,OV_val,config,timestamp,ch):
    ...
    odir = "%s/%s/%s/beam_run/run_ch%i_%s/OV%s/%s/"%( os.path.realpath(basedir), device_name,tileboard,ch,timestamp, OV_val, config_dict[config_name] )
    ...
    daqsocket.yamlConfig['daq']['menus']['externalL1A']['NEvents']=1000000
    ...
    for key in i2csocket.yamlConfig.keys():
        if key.find('roc_s')==0:
            nestedConf[key]['sc']['DigitalHalf']['all']['CalibrationSC'] = 1
            nestedConf[key]['sc']['DigitalHalf']['all']['L1Offset'] = 14 #13
            nestedConf[key]['sc']['Top']['all']['phase_ck']= 11
    i2csocket.update_yamlConfig(yamlNode=nestedConf.to_dict())
    i2csocket.configure()
    i2csocket.resettdc()
...
if __name__ == "__main__":
    ...
    positionData = pd.read_csv('hit_positions_showers.csv')
                ...
                for OV_val in OV:
                    ...
                    for config in configFile:
                        ...
                        odir = beam_run(i2csocket,daqsocket,clisocket,options.odir,options.dut,options.nruns,OV_val,config,timestamp,ch)
                            ...
                            #    try:
                            #        os.rename(
                            #            os.path.join(odir,"beam_run0.raw"),
                            #            os.path.join(odir,"beam_run0_failed%i.raw"%iteration)
                            #        )
                            #    except:
                            #        pass
