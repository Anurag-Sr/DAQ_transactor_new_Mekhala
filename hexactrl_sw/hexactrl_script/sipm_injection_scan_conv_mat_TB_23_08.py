...
def scan(i2csocket, daqsocket, injectedChannels, calib_dac_vals, gain, odir,keepRawData=0):
    ...
    for calibDAC_val in calib_dac_vals:
        ...
        i2csocket.configure_injection(trim_val = 0, process = 'int', calib_preamp = 0, calib_conv = calibDAC_val, gain=gain,injectedChannels=injectedChannels, IntCtest = 0, choice_cinj = 0, cmd_120p = 1, L_g2 = 1, H_g2 = 1, L_g1 = 0, H_g1 = 1, L_g0 = 1, H_g0 = 0)
...
if __name__ == "__main__":
    ...
    injectionConfig = {
        'BXoffset' : 22,   # was 23
        'gain' : 1,
        'phase' : 3,  #6 is optimal for toa threshold scan; 3 for tot 
        # 'calib' : [-1]+[i for i in range(0,4000,100)],
        'calib' : [i for i in range(0,2000,10)],   #was [i for i in range(0,4000,1)],
        'injectedChannels' : [2,38]
    }
