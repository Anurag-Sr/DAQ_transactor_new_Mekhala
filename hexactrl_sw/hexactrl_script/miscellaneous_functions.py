...
import os,datetime
'''
def mkdir(testName,suffix,basedir,device_type):
    directory_index = 1
...
def mkdir(*folder_args,testName,suffix):
    directory_index = 1
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    if suffix:
        timestamp = timestamp + "_" + suffix
    ...
    while True:
        odir = ""
        for args in folder_args:
            print("Argument name", args)
            odir += "%s/"%(args)
            
        odir += "%s_%s/"%(testName,directory_index)
        print("Output directory",odir) 
        dir_exist = os.path.exists(odir)
        if dir_exist:
            directory_index+=1
...
def options_sampling_scan_analyze():#No data taking   
    parser = options_run()
    parser.add_option("-n", "--directory_index", dest="directory_index", help="Number of runs for the device and device type")
    
    parser.add_option("--process", action="store", dest="process",default='', help="Type of injection (external or internal)")
    parser.add_option("--subprocess", action="store", dest="subprocess",default='', help="Type of internal injection (preamp or conveyor")
