...
class vrefinv_scan_analyzer(analyzer):
    ...
    def makePlots(self):
        ...
        for chip in range(nchip):
            ...
            for half in nhalf:
                ...
                ax.legend(handles=h,labels=l,loc='center right',ncol=2,bbox_to_anchor=(1.3, 0.5))


                plt.savefig("%s/pedestal_vs_vrefinv_chip%d_half%d.png"%(self.odir,chip,half),format='png',bbox_inches='tight') 
                plt.close()
            ####################################
    ...
    def fit(self, xy_df, x_name, y_name,half):
        #df = xy_df.groupby(x_name)[y_name].mean().reset_index()  # preprocess df
        df = xy_df[xy_df['half']==half].reset_index()
    ...
    def determine_bestVrefInv(self):
        ...
        data = data[(data['channel']!=17) & (data['channel']!=53) & (data['channeltype']==0)]
        yaml_dict={}
        ...
        for chip in range(nchip):
            if chip<len(rockeys):
                ...
                print(data)
                data_sorted = data.sort_values(by=["channel","Inv_vref"], ignore_index=True)
                print(data_sorted[data_sorted['channel']==0])
                means = data_sorted[ (data_sorted['chip']==chip)].groupby(['half','Inv_vref']).mean().reset_index()
                for half in range(2):
                    half_data_sorted = means[ means['half']==half ]
                    print(half_data_sorted)
                    xs, alpha, beta = self.fit( half_data_sorted, 'Inv_vref', 'adc_mean',half)
                    prof = half_data_sorted.groupby('Inv_vref')['adc_mean'].mean()
                    target = 200.0 #270.0 #50
                    ...
                    if alpha!=-1 and beta!=-1:
                        ...
                        print("Inv vref value for half",half," ",int( (target-beta)/alpha ))
