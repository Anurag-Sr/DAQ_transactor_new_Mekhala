...
def pedestal_scan(i2csocket,daqsocket, clisocket, basedir,device_name,suffix="",maindir=None):
    ...
    if maindir == None:
        maindir = "pedestal_scan"
    odir = "%s/%s/%s/run_%s/"%( os.path.realpath(basedir), device_name, maindir, timestamp ) # a comlete path is needed
    os.makedirs(odir)
...
if __name__ == "__main__":
    ...
    if options.initialize==True:
        ...
        i2csocket.configure()
    pedestal_scan(i2csocket,daqsocket,clisocket,options.odir,options.dut,suffix=options.suffix)
