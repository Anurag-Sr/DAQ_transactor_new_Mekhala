...
def tot_threshold_scan_sipm(i2csocket, daqsocket, clisocket, basedir, device_name, injectionConfig, suffix=''):
	...
	###############################################
        for ch in range(18):
                ...
                chan = ch
                injectedChannels = [chan,chan+18,chan+36,chan+36+18]
                scan(i2csocket, daqsocket, 560, 680, 2, injectedChannels, rundir,keepRawData=1)
                clisocket.stop()
        ...
        for folder in folders:
                ...
                for f in files[:]:
                        ...
                        ch = df_summary.injectedChannels.unique()[0]
                        df_raw = df_raw[df_raw.channel.isin([ch,ch+18])].copy()
                        df_.append(df_raw)
