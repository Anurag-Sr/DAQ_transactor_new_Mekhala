...
def pedestal_scan(i2csocket,daqsocket, clisocket, basedir,device_name,configFile):
    ...
    for chan in range(0,len(dataPd)):  
        ...
        intval = ((ped_needed-offset)/grad)
            
        if chan in calib_ch:    
            if intval >= 64:
                intval = 64
            cfg["roc_s0"]["sc"]["calib"][calib_i]["trim_inv"] = int(intval)
            ...
            if intval >= 64:
                intval = 64
            cfg["roc_s0"]["sc"]["cm"][cm_i]["trim_inv"] = int(intval)
            print("cm_i",cm_i)
            cm_i += 1
        else:
            try:  
                if intval >= 64 or math.isnan(intval):
                    intval = 64
                cfg["roc_s0"]["sc"]["ch"][chan_i]["trim_inv"] = int(intval)
                print("chan_i",chan_i)
            except OverflowError:
                cfg["roc_s0"]["sc"]["ch"][chan_i]["trim_inv"] = 0
                print("Overflow error, chan_i:"+str(chan_i)+" set to 0")
            chan_i += 1
            
        try:
            print(intval)
            print(grad*int(intval)+offset)   
        except OverflowError:
            pass
