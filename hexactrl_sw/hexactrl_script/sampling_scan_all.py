...
def scan(i2csocket, daqsocket, clisocket, index, out_txt_dir, scan_injection, process, subprocess, trim_val, gain, calib, active_menu, num_events, calibreq, startBX, stopBX, stepBX, startPhase, stopPhase, stepPhase, injectedChannels, odir):
    ...
    # --------------------
    util.saveFullConfig(odir=odir,i2c=i2csocket,daq=daqsocket,cli=clisocket,filename = 'data_taking_config.yaml')   
...
def Sampling_scan(i2csocket,daqsocket, clisocket, extra_text, basedir,device_name, device_type, injectionConfig, process = 'int', subprocess = 'preamp', suffix="", active_menu = 'calibAndL1A', num_events = 500, calibreq = 0x10, bxoffset = 21, noofoffsets = 5, stepBX = 1, startPhase=0, stopPhase=15, stepPhase=1):
    ...
    if process == 'ext':
        ...
        if subprocess == 'preamp':
            ...
            pre_injection = {'calib_preamp' : 0, 'calib_conv' : calib, 'IntCtest' : 0, 'choice_cinj' : 0, 'cmd_120p' : 1, 'L_g2' : 0, 'H_g2' : 1, 'L_g1' : 0, 'H_g1' : 1, 'L_g0' : 1, 'H_g0' : 0}
    ...
    fout.write('\n')
    fout.write("start of trigger pulse:" + '\n')
    fout.write(str(calibreq)+'\n')
    ...
    if process == 'int':
      scan(i2csocket=i2csocket, daqsocket=daqsocket, clisocket=clisocket, index = index, out_txt_dir = fout, scan_injection = scan_injection, process = process, subprocess = subprocess, trim_val = 0, gain = gain, calib = calib, active_menu = active_menu, num_events = num_events, calibreq = calibreq, startBX=startBX, stopBX=stopBX, stepBX=stepBX, startPhase=startPhase, stopPhase=stopPhase, stepPhase=stepPhase, injectedChannels=injectedChannels, odir=odir)
    elif process == 'ext':
      for trim_val in range(trimstart, trimstop, trimstep):
        scan(i2csocket=i2csocket, daqsocket=daqsocket, clisocket=clisocket, index = index, out_txt_dir = fout, scan_injection = scan_injection, process = process, subprocess = subprocess, trim_val = trim_val, gain = gain, calib = calib, active_menu = active_menu, num_events = num_events, calibreq = calibreq, startBX=startBX, stopBX=stopBX, stepBX=stepBX, startPhase=startPhase, stopPhase=stopPhase, stepPhase=stepPhase, injectedChannels=injectedChannels, odir=odir)
...
def Sampling_scan_analysis(i2csocket,process,subprocess,basedir,device_name,device_type,directory_index, calib, LEDvolt, OV, cfgF, suffix=""): #This is already agnostic of the process, even the analysis file used is the same
...
#There is no need to give the full injectionConfig here, only the list of channels for plotting
    ...
    odir = "%s/%s/%s_%s/"%( os.path.realpath(basedir),device_type,testName,directory_index)
    print("Directory to be analyzed", odir)
    
    scan_analyzer = analyzer.overall_analyzer(odir=odir)
    ...
    no_phase_chan = scan_analyzer.channel_sampling_scan_internal_check("TB3_D8",injectedChannels,len(files),odir,process='int',subprocess='preamp',height_percent=0.1,config_file = cfgF,fout = odir + "analysis_summary_new.yaml")
    print("List of channels excluded from phase calculation", no_phase_chan)
    scan_analyzer.determine_bestPhase(injectedChannels,odir, no_phase_chan)    
