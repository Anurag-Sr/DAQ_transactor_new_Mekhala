...
def sampling_scan(i2csocket,daqsocket, clisocket, basedir,device_name, injectionConfig,suffix=""):
    ...
    odir = "%s/%s/sampling_scan_ext/TB3_1_LED_scan_Rf14_ConvGain4_OV%iV_%imV_%s/"%( os.path.realpath(basedir), device_name, voltages, LEDvolt, timestamp) 
    os.makedirs(odir)
    ...
    calibreq = 0x10
    bxoffset = 24 #  --------24 in mathias's script
    startBX=calibreq+bxoffset-2 #----------calibreq+bxoffset-1 in Mathias's script
...
if __name__ == "__main__":
    ...
    injectionConfig = {
        ...
        #'injectedChannels' : [10],
        'LEDvolt' : 5500,  # LED_BIAS (LED amplitude) in mV  #------------added for sampling scan ext
