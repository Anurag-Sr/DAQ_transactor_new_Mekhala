...
OV = ["4V"]
convGain = "4" #ConvGain
tileboard = 'TB3_D8_ps1'
testName = "beam_run"
channel = 'test' #=================================================change manually for the next 500 years
...
configs = [
    ...
    #"sipm_roc0_onbackup0_gainconv4_Dacb_vb_conv5_trimtoa_trimtot_after_pedestal_adjustment.yaml"
    #"convgain4_pedestal_aligned.yaml"
    #"roc_config_ConvGain12_allchs.yaml"
    "sipm_roc0_onbackup0_gainconv12_pre_series_TB_testing.yaml"
...
OV_dict = {
    ...
    "TB3_D8_1": {
        ...
        '4V': {'A':196,'B':122},#from Aug 2023 test beam, MPPC_BIAS1 = 43.98
     ...
     "TB3_D8_ps1": {
        '2V': {'A':185,'B':125,'C':190,'D':115},
        '4V': {'A':196,'B':122,'C':200,'D':118},
     },
     "TB3p1_D8_Board1": {
        '0V9': {'A':188,'B':136},  # 40.5
        '1V': {'A':189,'B':134},  # 40.6
...
def beam_run(i2csocket,daqsocket, clisocket, basedir,device_name, nruns,OV_val,config,timestamp):
    ...
    if tileboard=="TB3_2":
        ...
        print("So............The tileboard us not equal TB3_2")
    ...
    if (diff>0):
        # "sleep" is only required when SiPM bias voltage is changed:
        print(" ")
        print(" Please wait for voltage stabilization (5 seconds)")
    ...
    for key in i2csocket.yamlConfig.keys():
        if key.find('roc_s')==0:
            ...
            nestedConf[key]['sc']['DigitalHalf']['all']['L1Offset'] = 13 #13 #=============================================change manually
            nestedConf[key]['sc']['Top']['all']['phase_ck']= 11 #=============================================change manually
    i2csocket.update_yamlConfig(yamlNode=nestedConf.to_dict())
    i2csocket.configure()
    print("i2csocket.resettdc() start from beam run script")
    i2csocket.resettdc()
    print("i2csocket.resettdc() end from beam run script")
...
if __name__ == "__main__":
    ...
    for OV_val in OV:
        ...
        for config in configFile:
            ...
            print("Start the beam run from beam run script")
            odir = beam_run(i2csocket,daqsocket,clisocket,options.odir,options.dut,options.nruns,OV_val,config,timestamp)
            print("Finished the beam run from beam run script")
