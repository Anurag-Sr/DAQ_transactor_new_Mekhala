...
class LinkBuilder:
    def __init__(self):
        ...
        self.sca = gbtsca('sca0', {'address': 0x00})
        self.sca.set_transport(transport)
        self.sca.reset()
        self.sca.enableADC()
        self.sca.enableDAC()
...
gpio_tbtester_map = {
    'roc_s0': [
        ('hgcroc_soft_rstB', 0x4),
        ('hgcroc_i2c_rstB',  0x8),
        ('hgcroc_hard_rstB', 0x10)
    ]
}
gpio_tbtester_noresetmap = {
    'roc_s0': [
        ('EN_LDO', 0x400000),
        ('SOFTSTART', 0x800000)
    ]
...
def sca_gpio_discover(sca: gbtsca):
    lines = []
    sca.enableGPIO(True)
    for name, mask in gpio_tbtester_map['roc_s0']:
        lines.append(sca.gpio(name, mask, False))
    for name, mask in gpio_tbtester_noresetmap['roc_s0']:
        line = sca.gpio(name, mask, False)
        line.set_value(1)
